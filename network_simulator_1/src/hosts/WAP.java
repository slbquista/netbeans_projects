...
public class WAP {
    ...
    public WAP() throws ClassNotFoundException, SQLException {
        this.nd = NetworkDevices.getInstance();
        
        for (int i = 0; i < nd.getNetworkList().size(); i++) {
            if (nd.getNetworkList().get(i).getNodename().equalsIgnoreCase("wap1")) {
    ...
    public void turnOn() {
        for (int i = 0; i < nd.getNetworkList().size(); i++) {
            if (nd.getNetworkList().get(i).getNodename().equalsIgnoreCase("wap1") && nd.getNetworkList().get(i).isOnline() == false) {
    ...
    public void turnOff() {
        for (int i = 0; i < nd.getNetworkList().size(); i++) {
            if (nd.getNetworkList().get(i).getNodename().equalsIgnoreCase("wap1") && nd.getNetworkList().get(i).isOnline() == true) {
